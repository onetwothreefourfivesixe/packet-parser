...
def convertFile():
    ...
    for file_path in os.listdir("p-pdf"):
        fileName = file_path.lower()
        file_path = Path("p-pdf") / file_path
        print(f"Parsing {file_path}...")
        counter += 1
        basename, file_type = os.path.splitext(file_path)
        print(file_type)
        if file_type == ".pdf":
            ...
            output_path = Path("packets") / fileName.replace('.pdf', '.txt')
            ...
            output_path = Path("packets") / fileName.replace('.docx', '.txt')
            # Write with UTF-8 encoding
            output_path.write_text(text_content, encoding='utf-8', errors='replace')
            
        elif file_type == ".txt":
            output_path = Path("packets") / fileName
